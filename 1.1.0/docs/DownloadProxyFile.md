...
inputs:
  ...
  - name: Asset id
    description: |
      Enter the id of the asset to download the proxy file of
    type: STRING
    mandatory: true
    example:
      - name: Asset id
...
outputs:
  - name: File path
    ...
    example:
      - name: File path
        value: "/Users/helmut/downloads/lower-third-01-proxy.mp4"
  - name: File size
    description: |
...
connectors:
  ...
  - name: Fail
    ...
    causes:
      - name: File Not Found
        description: |
          If Vulcano holds no proxy file for the given Asset id (404), which also happens while the asset is still rendering
