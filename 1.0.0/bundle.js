...
function hasStatus(message, status) {
...
async function localFileSize(action, inputName, filePath) {
  try {
    return (await (0, import_promises.stat)(filePath)).size;
  } catch {
...
async function skippableFile(requestedPath, option) {
  if (option !== import_DuplicateFileOptionEnum.DuplicateFileOption.SKIP) return void 0;
  try {
    await (0, import_promises.access)(requestedPath);
    return { filePath: requestedPath, fileSize: (await (0, import_promises.stat)(requestedPath)).size };
  } catch {
    return void 0;
  }
}
async function streamToFile(wave, response, finalPath) {
  ...
  await (0, import_promises2.pipeline)(response.data, (0, import_node_fs.createWriteStream)(finalPath));
  return bytesReceived;
}
async function downloadAssetFile(wave, options) {
  const requestedPath = import_node_path.default.join(options.targetFolder, options.fileName);
  ...
  try {
    ...
    throw new Error(
      `${options.action} \u2014 ${requestedPath} could not be created: ${err?.message ?? "unknown error"} \u2014 verify Target folder is writable and check Duplicate file option`
    );
  }
  const controller = new AbortController();
  let cancelInterval;
  try {
    ...
    await (0, import_promises.unlink)(finalPath).catch(() => void 0);
...
var CreateGraphic = class extends Node {
  ...
  async execute() {
    ...
    const outputDurationSeconds = this.wave.inputs.getInputValueByInputName("Output duration seconds" /* OUTPUT_DURATION_SECONDS */);
...
var CreateOgrafLink = class extends Node {
  ...
  async execute() {
    ...
    const expiryDays = this.wave.inputs.getInputValueByInputName("Link expiry days" /* LINK_EXPIRY_DAYS */);
...
var CreateVggProject = class extends Node {
  ...
  async execute() {
    ...
    let baseVideoPath;
    try {
      baseVideoPath = await this.wave.axiosHelper.makeRequest(
        vulcanoRequest(baseUrl, apiToken, {
          method: "POST",
    ...
    const saveConfig = vulcanoRequest(baseUrl, apiToken, {
      method: "PUT",
...
var ListTemplates = class extends Node {
  ...
  async execute() {
    ...
    const maxResults = this.wave.inputs.getInputValueByInputName("Max results" /* MAX_RESULTS */);
    const sortBy = this.wave.inputs.getInputValueByInputName("Sort by" /* SORT_BY */);
    const sortDirection = this.wave.inputs.getInputValueByInputName("Sort direction" /* SORT_DIRECTION */);
    const requestConfig = vulcanoRequest(baseUrl, apiToken, {
      method: "GET",
      url: "/assets",
...
function assertMogrtPath(filePath) {
  if (import_node_path3.default.extname(filePath).toLowerCase() !== ".mogrt") {
...
var UploadMogrt = class extends Node {
  ...
  async execute() {
    ...
    this.wave.logger.updateProgressAndMessage(0, `Uploading ${import_node_path3.default.basename(mogrtFilePath)}`);
    try {
      await this.wave.axiosHelper.makeRequest(
        vulcanoRequest(baseUrl, apiToken, {
          method: "POST",
          url: "/files",
    ...
    this.wave.outputs.setOutput("Uploaded file name" /* UPLOADED_FILE_NAME */, import_node_path3.default.basename(mogrtFilePath));
    this.wave.outputs.setOutput("File size" /* FILE_SIZE */, fileSize);
  }
};
